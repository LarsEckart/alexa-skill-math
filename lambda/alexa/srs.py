...
class SpacedRepetition:
    ...
    def _should_generate_new(self) -> bool:
        ...
        available = [
            q_id for q_id in self._stats
            if q_id not in self._recent_questions
        ]
    ...
    def _select_from_srs(self) -> MathQuestion | None:
        ...
        available_stats = [
            stats for q_id, stats in self._stats.items()
            if q_id not in self._recent_questions
        ]
    ...
    def get_session_stats(self) -> dict:
        ...
        for q_id in self._session_asked:
            if q_id in self._stats:
                stats = self._stats[q_id]
                # Count only the last answer in this session
                session_total += 1
                # We assume the last answer is what updated the stats
                # This is a simplification; for accurate session tracking,
                # we'd need to track session answers separately
